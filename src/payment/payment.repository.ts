...
import { Payment, PaymentDocument, PaymentType } from './entities/payment.entity';
...
export class PaymentRepository extends BaseAbstractRepository<Payment> {
  ...
  public async allExpensisMony(department: string) {
    let stages = [
      {
        $match: {
  ...
  public async allExpensisMonyApp() {
    let stages = [
      {
        $match: {
          paymentType: PaymentType.EXPENSIS,
  ...
  public async allRevenueMonyApp() {
    let stages = [
      {
        $match: {
  ...
  public async allRevenueMony(department: string) {
    let stages = [
      {
        $match: {
          department: ObjectId(department),
